...
public class TransparentDetection : MonoBehaviour
{
    ...
    [SerializeField] private float faceTime = .4f;
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.gameObject.GetComponent<PlayerController>()){
            //fade the object
            if(spriteRenderer){
                StartCoroutine(FadeRoutine(spriteRenderer, faceTime, spriteRenderer.color.a, transparencyAmount));
            } else if (tilemap){
                StartCoroutine(FadeRoutine(tilemap, faceTime, tilemap.color.a, transparencyAmount));
            }
        }    
    }
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
       if (other.gameObject.GetComponent<PlayerController>()){
            ...
            if(spriteRenderer){
                ...
                StartCoroutine(FadeRoutine(tilemap, faceTime, tilemap.color.a, 1f));
            }
       } 
    }
    ...
    private IEnumerator FadeRoutine(SpriteRenderer spriteRenderer, float fadeTime, float startValue, float targetTransparency){
        ...
        while(elasptime < fadeTime){
            ...
            float newAlpha = Mathf.Lerp(startValue, targetTransparency, elasptime / fadeTime);
